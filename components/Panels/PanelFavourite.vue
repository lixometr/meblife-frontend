<template>
  <Panel
    ...
    @close="$emit('close')"
  >
    <template #title>{{$t('favourite')}}</template>
    <template #content>
              ...
              <div class="flex">
                ...
                >
                  <div class="position-relative w-100 h-100">
                    <AppImage contain absoluteFull noBg v-bind="item.default_image" />
                  </div>
                </div>
                <div class="panel-favourite__item-content text-right">
                  <div
                    ...
                    @click="openProduct(item.slug)"
                  >{{item.full_name}}</div>
                  <div class="panel-favourite__item-delivery mt-3" v-if="item.delivery_24">
                    {{$t('deliveryText')}}
                    <b class="color-green">{{$t('delivery24')}}</b>
                  </div>
                  <div class="panel-favourite__item-delivery mt-3" v-else>
                    {{$t('deliveryText')}}
                    <b>{{item.delivery_days}} {{deliveryText(item.delivery_days)}}</b>
                  </div>
                </div>
              </div>
              <div class="flex align-end justify-between mt-2">
                <div class="panel-favourite__item-actions flex">
                  <div class="btn btn-circle btn-white mr-2" @click="removeItem(item._id)">
                    <svgTrash width="20" />
                  </div>
                  <div class="btn btn-circle btn-white mr-2" @click="addToCart(item._id)">
                    <svgCart width="20" />
                  </div>
                </div>
                <div class="panel-favourite__item-price text-right">
                  <s class="text-14 mb-1" v-if="item.old_price">{{item.old_price}} {{currency}}</s>
